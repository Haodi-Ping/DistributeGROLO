...
class Robot(object):
    ...
    def get_coord(self):
        if self.sess != None:
            tmpcoord = self.sess.run(self.coord)
            return [tmpcoord[0], tmpcoord[1]]
        else:
            return self.distribute_temp_coord

    def set_coord(self, x, y):
        if self.sess != None:
            print('the x and y is ',x,y)
            self.sess.run(self.coord.assign([float(x), float(y)]))
        else:
            self.distribute_temp_coord = [x,y]
